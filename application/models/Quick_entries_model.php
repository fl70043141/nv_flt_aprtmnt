...
class Quick_entries_model extends CI_Model
{
	function __construct(){	
        ...
        public function search_result2($data=''){ 
//            echo '<pre>';            print_r($data); die;
            $this->db->select('q.*'); 
            $this->db->from(GL_MENTEN_ENTRY.' q');     
            $this->db->where('q.deleted',0); 
            if($data!=''){
//                if($data['from_date']!='' && $data['to_date']!='') $this->db->where("q.bill_month>= ".$data['from_date']." AND q.bill_month<= ".$data['to_date']." ");
...
//            echo $this->db->last_query();die;
         ...
         public function get_single_row2($id,$where=''){ 
            $this->db->select('q.*'); 
            $this->db->from(GL_MENTEN_ENTRY.' q');     
            $this->db->where('q.deleted',0); 
...
//            echo $this->db->last_query();die;
         ...
         public function get_entry_desc($id){ 
            $this->db->select('id.*');
            $this->db->select('(select account_name from '.GL_QUICK_ENTRY_ACC.' where id = id.quick_entry_account_id) as account_name');
            $this->db->from(GL_QUICK_ENTRY.' id'); 
            $this->db->where('id.maintenance_id',$id);
            $this->db->where('id.deleted',0);
            $result = $this->db->get()->result_array(); 
            return $result;
	}  
...
//            echo '<pre>';            print_r($data); die;   
            ...
            if(!empty($data['main_tbl']))$this->db->insert(GL_MENTEN_ENTRY, $data['main_tbl']); 
...
//            echo '<pre>'; print_r($data); die;
		...
		$this->db->where('id', $id); 
                $this->db->where('deleted',0);
		$this->db->update(GL_MENTEN_ENTRY, $data);
